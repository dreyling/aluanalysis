...
# scattering angle, x-axis data
if len(sys.argv) < 3:
  print "please select x data..."
  print data.dtype
  exit()
scattering_xdata = sys.argv[2]

# scattering angle, y-axis data
if len(sys.argv) < 4:
  print "please select y data..."
  print data.dtype
  exit()
scattering_ydata = sys.argv[3]
...
for index, thickness in enumerate(thicknesses):
  ...
  ax1.plot(data[cut][scattering_xdata], data[cut][scattering_ydata], 
			color='k', 
...
ax1.set_ylabel(scattering_ydata.replace("_", " "))
