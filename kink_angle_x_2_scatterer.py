...
for index, value in enumerate(newlist):
	...
	specs = mrl.getHistSpecs(runlist, index, histname, name_path_2scatterer, name_suffix_2scatterer, name_rootfolder)
	#print specs['entries']
	newlist['proc_events'][index] = specs['entries']
	#print specs['stddev']
	newlist['rmsROOT'][index] = specs['stddev']
	# 3. add rms98
	data = mrl.getHist1Data(runlist, index, histname, name_path_2scatterer, name_suffix_2scatterer, name_rootfolder)
