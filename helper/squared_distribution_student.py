...
# data 
degfree = 3
data = np.random.standard_t(degfree, 10000000)
#print data[0]
...
print "normal distribution width", st_std
sq_mean = mdp.calc_hist_mean(np.vstack((bins2_cent, count2))) 
print "squared distribution mean", sq_mean, math.sqrt(sq_mean)
...
# text
ax1.text(0.05, 0.9, 'RMS: {:.3f}'.format(st_std), transform=ax1.transAxes, fontsize=10, verticalalignment='top', horizontalalignment='left')
ax2.text(0.05, 0.9, 'Mean: {:.3f} \nsqrt(Mean): {:.3f}\nratio: {:.2f}'.format(sq_mean, math.sqrt(sq_mean), st_std/math.sqrt(sq_mean)), transform=ax2.transAxes, fontsize=10, verticalalignment='top', horizontalalignment='left')
